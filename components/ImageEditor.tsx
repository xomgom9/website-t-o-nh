...
export const ImageEditor: React.FC<ImageEditorProps> = ({ imageUrl, productAssets = [], onSave, onCancel }) => {
  ...
  return (
    <div className="flex h-[80vh] bg-slate-900 rounded-2xl overflow-hidden border border-slate-800 shadow-2xl">
      <div className="w-16 bg-slate-800/80 border-r border-slate-700 flex flex-col items-center py-6 space-y-6">
        <button onClick={() => setSmartTool(smartTool === 'swap' ? 'none' : 'swap')} className={`p-3 rounded-xl transition ${smartTool === 'swap' ? 'bg-indigo-600 text-white' : 'bg-slate-700 text-slate-400 hover:text-white'}`} title="Smart Swap Product">
        ...
        <button onClick={() => setSmartTool(smartTool === 'erase' ? 'none' : 'erase')} className={`p-3 rounded-xl transition ${smartTool === 'erase' ? 'bg-red-600 text-white' : 'bg-slate-700 text-slate-400 hover:text-white'}`} title="Magic Eraser">
      ...
      <div className="flex-1 bg-slate-950 relative flex items-center justify-center p-8 overflow-hidden">
        {isProcessing && <div className="absolute inset-0 z-50 bg-black/60 flex items-center justify-center"><div className="w-10 h-10 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin" /></div>}
        ...
        {smartTool === 'swap' && (
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-slate-900 border border-indigo-500/50 p-6 rounded-2xl shadow-2xl w-[90%] max-w-xl animate-bounce-in">
            <h4 className="text-sm font-bold text-indigo-400 mb-4 uppercase tracking-widest">Select Product Asset to Insert</h4>
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
              {productAssets.length === 0 ? <p className="text-xs text-slate-500">No assets available. Generate some in Product Mode first.</p> : productAssets.map(a => (
                <button key={a.id} onClick={() => handleAssetSwap(a.url)} className="flex-shrink-0 w-24 h-24 rounded-lg border-2 border-slate-800 hover:border-indigo-500 overflow-hidden bg-black/40 transition">
                  <img src={a.url} className="w-full h-full object-contain" />
                </button>
              ))}
            </div>
            <div className="mt-4 flex gap-4">
               <input type="text" value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} placeholder="Instruction (e.g., blend with shadows)..." className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-xs" />
      ...
      <div className="w-72 bg-slate-900 border-l border-slate-800 p-6 flex flex-col">
          <div className="flex justify-between items-center mb-8">
              <h3 className="text-xs font-bold text-slate-500 uppercase">Editor Controls</h3>
